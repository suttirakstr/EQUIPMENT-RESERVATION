...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif ($role == "user" && !preg_match("/^[0-9-]{14}$/", $_POST['idstudent'])) {
        $_SESSION['error'] = 'รหัสนักศึกษาควรประกอบด้วยตัวเลขและเครื่องหมายลบ (-) เท่านั้น';
    }else {
        $query = $conn->prepare("UPDATE users SET prefix = :prefix, firstname = :firstname, lastname = :lastname, idstudent = :idstudent, phone = :phone, nameteacher = :nameteacher, email = :email WHERE id = :id");
        $query->execute(array(':id' => $id, ':prefix' => $prefix, ':firstname' => $firstname, ':lastname' => $lastname, ':idstudent' => $idstudent, ':phone' => $phone, ':nameteacher' => $nameteacher, ':email' => $email));
                                        ...
                                        <div class="form-group row">
                                                <?php if ($role == 'user'): ?>
                                                    <div class="input-group mb-2">
                                                        ...
                                                        <select class="form-select col-sm-3" id="inputGroupSelect01"
                                                            ...
                                                            <option value="<?= $data['prefix'] ?>" ><?php echo $data['prefix']?></option>
                                            ...
                                            <?php if ($role == 'admin' || $role == 'teacher'): ?>
                                                ...
                                                <label class="input-group-text "
                                                    ...
                                                    ?>
                                                    <select class="form-select col-sm-3" id="inputGroupSelect01" name="prefix" aria-describedby="prefix">
                                                        <option value="<?= $data['prefix'] ?>"  class="col-lg-5"> <?php echo $data['prefix'] ?></option>
                                            ...
                                            <div class="col-sm-3 ">
                                                ...
                                                <div>
                                                    <input type="text" class="form-control" value="<?= $data['firstname'] ?>"
                                                        aria-label="firstname" aria-describedby="basic-addon1" name="firstname">
                                                </div>
                                                <div class="col-sm-2 ">
                                                ...
                                                <div>
                                                    <input type="text" class="form-control" value="<?= $data['lastname'] ?>"
                                                        aria-label="lastname" aria-describedby="basic-addon1" name="lastname">
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                            <?php if ($role == 'admin'|| $role == 'teacher'):  ?>
                                                <div class="col-sm-3 ">
                                                 <h4 class="textcolor">ชื่อผู้ใช้งาน:</h4>
                                                 </div> 
                                                 <div>
                                                    <input type="text" class="form-control" value="<?= $data['idstudent'] ?>"
                                                        aria-label="idstudent" aria-describedby="basic-addon1" name="idstudent">
                                                </div>       
                                                <?php endif; ?>
                                                <?php if ($role == 'user'):  ?>
                                                <div class="col-sm-3 ">
                                                 <h4 class="textcolor">รหัสนักศึกษา:</h4>
                                                 </div> 
                                                 <div>
                                                    <input type="text" class="form-control" value="<?= $data['idstudent'] ?>"
                                                        aria-label="idstudent" aria-describedby="basic-addon1" name="idstudent">
                                            ...
                                            <?php if ($role == 'user'): ?>
                                                    ...
                                                    $datas = $stmt->fetchAll(PDO::FETCH_ASSOC);
                                                    ?>
                                                    <select class="form-select col-sm-4" id="inputGroupSelect01" name="nameteacher" aria-describedby="nameteacher">
                                                        <option value="<?= $data['nameteacher'] ?>"  class="col-lg-5"> <?php echo $data['nameteacher'] ?></option>
                                                        <?php foreach ($datas as $row): ?>
                                                        <option >
