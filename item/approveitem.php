...
$prefix_firstname_lastname = $row['prefix'].$row['firstname']. ' ' .$row['lastname'];
$statusArray = array("รอการอนุมัติ");
$statusPlaceholders = implode(",", array_fill(0, count($statusArray), "?"));
$sql = "SELECT * FROM approve WHERE status IN ($statusPlaceholders) 
AND nameteacher = ?";
...
$stmt->bindValue(count($statusArray) + 1, $prefix_firstname_lastname, PDO::PARAM_STR);
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = $conn->prepare("UPDATE approve SET status = :status WHERE id = :id");
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">ตารางขอยืมพัสดุทั้งหมด</h1>
                    <div class="card shadow mb-4">
                        <div class="form-group row">
                            <div class="card-header py-3 col-md-10">
                                <h6 class="m-0 font-weight-bold text-primary">
                                    รายชื่อพัสดุที่ทำการขออนุมัติการยืม
                                </h6>
                            </div>
                                    ...
                                    <tbody id="table-body">
                                        <?php foreach ($data as $s): ?>
                                            <tr>
                                                ...
                                                <td><button type="button" class="btn btn-success btn-sm" onclick="window.location.href='approve.php?id=<?php echo $s['id'];?>'">
                                                        <a>อนุมัติการยืม</a>
                                                    </button>
                                </td>
                                </tr>
