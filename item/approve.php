...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['status'])) {
        ...
        if ($selectedStatus === 'ไม่ได้รับการอนุมัติ') {
            // Implement the logic for "ว่างไม่มีการใช้งาน"
            $status = $_POST['status'];
            $recordId = $_POST['record'];
            $quantityReturned = $_POST['units'];

            // Define and implement the returnMaterial function
            if (returnMaterial($recordId, $quantityReturned)) {
                header('Location: approveitem.php');
                echo "<p>คืนวัสดุสำเร็จ</p>";
            } else {
                echo "<p>เกิดข้อผิดพลาดในการคืนวัสดุ</p>";
            }
        } elseif ($selectedStatus === 'รอการรับพัสดุ') {
            // Implement the logic for "รอการรับพัสดุ"
            $status = $_POST['status'];
            ...
            // Assuming $id contains the appropriate value
            $query = $conn->prepare("UPDATE approve SET status = :status, firstname = :firstname, lastname = :lastname, nameitem = :nameitem, typeitem = :typeitem, nameteacher = :nameteacher, units = :units WHERE id = :id");
            $query->execute(array(':id' => $id, ':status' => $status, ':firstname' => $firstname, ':lastname' => $lastname, ':nameitem' => $nameitem, ':typeitem' => $typeitem, ':nameteacher' => $nameteacher, ':units' => $units));
            ...
            $_SESSION['error'] = "<p>กรุณาเลือกการอนุมัติ</p>";
...
<head>
    ...
    <title>ข้อมูลการอนุมัติการยืม</title>
                                ...
                                <div class="col-lg-12">
                                    <div class="p-5">
                                        <div class="text-center">
                                            <h1 class="h4 text-gray-900 mb-5">ข้อมูลการยืม</h1>
                                        </div>
                                        <form class="user" action="" method="POST">
                                            ...
                                            function returnMaterial($recordId, $quantityReturned)
                                            {
                                                ...
                                                $stmt = $conn->prepare("UPDATE approve SET status = 'ไม่ได้รับการอนุมัติ', return_date = CURRENT_TIMESTAMP() WHERE id = ?");
                                                $stmt->execute([$recordId]);
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <label for="nameitem"
                                                    class="col-form-label">ชื่ออาจารย์ที่ปรึกษา:</label>
                                                <div class="col-md-5">
                                                    <input type="text" class="form-control"
                                                        value="<?php echo $data['nameteacher']; ?>"
                                                        aria-label="nameteacher" aria-describedby="nameteacher"
                                                        name="nameteacher" readonly>
                                                </div>
                                                <label for="nameitem" class="col-form-label">จำนวนพัสดุ:</label>
                                                <div class="col-md-3">
                                                    <input type="hidden" value="<?= $data['id'] ?>" name="record"
                                                        id="record">
                                                    <input type="text" class="form-control"
                                            ...
                                            <div class="input-group col-mb-3">
                                                ...
                                                <select class="form-select col-sm-6" id="inputGroupSelect01"
                                                    name="status" aria-describedby="status">
                                                    <!-- <option disabled selected class="col-lg-5">
                                                        <?php echo $data['status'] ?>
                                                    </option> -->
                                                    <option value="รอการรับพัสดุ">อนุมัติ</option>
                                                    <option value="ไม่ได้รับการอนุมัติ">ไม่อนุมัติ</option>
                                                </select>
                                            </div>
                                            <hr>
