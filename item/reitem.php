...
$statusArray = array("กำลังใช้งาน");

// สร้างคำสั่ง SQL โดยใช้ IN ในการเลือกสถานะ
$statusPlaceholders = implode(",", array_fill(0, count($statusArray), "?"));
$sql = "SELECT * FROM approve WHERE status IN ($statusPlaceholders) AND firstname = ?";

$stmt = $conn->prepare($sql);
foreach ($statusArray as $key => $status) {
    $stmt->bindValue($key + 1, $status, PDO::PARAM_STR);
}
$stmt->bindValue(count($statusArray) + 1, $firstname, PDO::PARAM_STR);
$stmt->execute();
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800">ตารางคืนพัสดุ</h1>
                    <!-- DataTales Example -->
                    <div class="card shadow mb-4">
                                    ...
                                    <table class="display table table-bordered my-4" id="example" width="100%"
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="col-md-1" scope="col">คืนพัสดุ</th>
                                            </tr>
                                        </thead>
                                        <tbody id="table-body">
                                            <?php foreach ($data as $s): ?>
                                                <tr>
                                                    ...
                                                    <td ><button type="button" class="btn btn-success btn-sm"
                                                            onclick="window.location.href='approveitemadmin1.php?id=<?php echo $s['id']; ?>'">
                                                            <a>คืนพัสดุ</a>
                                                        </button>
                                                    </td>
                                                </tr>
                        ...
                        <button class="btn btn-primary "><a href="myhistory.php"
                                class="text-white">ดูประวัติการทำรายการพัสดุ</a></button>
                            <div class="my-4"></div>                     
